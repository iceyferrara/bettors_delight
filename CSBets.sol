...
//  string public testString = "json(https://api.pandascore.co/dota2/matches.json?filter[id]=52365&token=tU9uGM46ds_tXnE6FkW3u9g43EV1HsfuXOBPVNkmPHOBzMDK13Q).0.winner.name";
  ...
  function distanceRetrieve(uint _matchID) payable {
        bytes32 queryId = oraclize_query("URL",  strConcat(firstHalfQuery, uint2str(_matchID), secondHalfQuery));
        oraclizeCallbacks[queryId] = oraclizeCallback(oraclizeState._matchID);
  }
  ...
  function fetchMatchInfo(uint _matchID) onlyOwner payable{
    oraclizeID = oraclize_query("URL",  strConcat(firstHalfQuery, uint2str(_matchID), secondHalfQuery));
  }
